...
import io.github.C0000374.MaC_Patched.Hacks.CLockSlot;
...
import net.skidcode.gh.maybeaclient.hdtexture.CustomAnimation;
...
public abstract class Minecraft implements Runnable {
    ...
    public void reloadFXTextures() {
		this.renderEngine.textureList.clear();
		boolean custom = !(this.texturePackList.selectedTexturePack instanceof TexturePackDefault) ? true : false;
		if(custom) {
			this.renderEngine.registerTextureFX(this.textureWaterFX = new CustomAnimation(12 * 16 + 13, 0, 1, "water_still", -1, -1));
			this.renderEngine.registerTextureFX(this.textureLavaFX = new CustomAnimation(14 * 16 + 13, 0, 1, "lava_still", -1, -1));
			if(Client.textureExists("/anim/custom_fire_e_w.png") && Client.textureExists("/anim/custom_portal.png")) {
				this.renderEngine.registerTextureFX(new CustomAnimation(0 * 16 + 14, 0, 1, "portal", -1, -1));
			}else {
			...
			this.renderEngine.registerTextureFX(new CustomAnimation(12 * 16 + 13+1, 0, 2, "water_flowing", 0, 0));
			this.renderEngine.registerTextureFX(new CustomAnimation(14 * 16 + 13+1, 0, 2, "lava_flowing", 3, 6));
			
			if(Client.textureExists("/anim/custom_fire_e_w.png") && Client.textureExists("/anim/custom_fire_n_s.png")) {
		   	 this.renderEngine.registerTextureFX(new CustomAnimation(1 * 16 + 15 + 16, 0, 1, "fire_n_s", 2, 4));
				this.renderEngine.registerTextureFX(new CustomAnimation(1 * 16 + 15, 0, 1, "fire_e_w", 2, 4));
			}else {
			...
			this.renderEngine.registerTextureFX(this.textureWaterFX = new TextureWaterFX());
			this.renderEngine.registerTextureFX(this.textureLavaFX = new TextureLavaFX());
			this.renderEngine.registerTextureFX(new TexturePortalFX());
			this.renderEngine.registerTextureFX(new TextureCompassFX(this));
			this.renderEngine.registerTextureFX(new TextureWatchFX(this));
    ...
    public void startGame() throws LWJGLException {
        ...
        if(Client.debug) ; //resize call to fix fbs
		...
		this.reloadFXTextures(); //XXX HD Textures
    ...
    public void setIngameNotInFocus() {
        if (this.inGameHasFocus) {
            if (this.thePlayer != null) {
            	if (!InventoryWalkHack.instance.status || !(this.currentScreen instanceof GuiContainer)) {
    ...
    private void clickMiddleMouseButton() {
        if (this.objectMouseOver != null) {
        	...
        	if(ForceFieldHack.instance.middleClickFriend.getValue()) {
				if(this.objectMouseOver.typeOfHit == EnumMovingObjectType.ENTITY && this.objectMouseOver.entityHit instanceof EntityPlayer) {
					EntityPlayer p = (EntityPlayer) this.objectMouseOver.entityHit;
					ForceFieldHack.instance.ignoreList.setValue(p.username);
					if(ForceFieldHack.instance.notifyInChat.getValue()) {
						Client.addMessage(ChatColor.GOLD+""+p.username+ChatColor.WHITE+" was "+(ForceFieldHack.instance.ignoreList.contains(p.username) ? "added into" : "removed from")+" Killaura's Ignore List!");
					}
					Client.saveModules();
				}
			}
    ...
    public void runTick() {
        ...
        if (CLockSlot.Instance.status && this.thePlayer != null) this.thePlayer.inventory.currentItem = CLockSlot.Instance.Slot.value;
                                        ...
                                        do {
                                            if (!Keyboard.next()) {
                                                if (this.currentScreen == null) {
                                                    ...
                                                    if (Mouse.isButtonDown(1) && (FastPlaceHack.instance.status || (float)(this.ticksRan - this.mouseTicksRan) >= this.timer.ticksPerSecond / 4.0F) && this.inGameHasFocus) {
                                                        this.clickMouse(1);
                                                        this.mouseTicksRan = this.ticksRan;
                                                ...
                                                AFKDisconnectHack.stopAFKing();
                                                this.func_6254_a(0, this.currentScreen == null && (Mouse.isButtonDown(0) || (AutoMouseClickHack.instance.status && AutoMouseClickHack.instance.left.currentMode.equalsIgnoreCase("Hold"))) && this.inGameHasFocus);
                                                break label301;
                                            }
                                        ...
                                        if (Keyboard.getEventKey() == 87) {
                                            ...
                                            if (this.currentScreen != null) {
                                                ...
                                                if (Keyboard.getEventKey() == 63) {
                                                	if(ThirdPersonTweaksHack.instance.status && ThirdPersonTweaksHack.instance.frontView.currentMode.equalsIgnoreCase("ThirdF5Mode")) {
														if(!this.gameSettings.thirdPersonView) {
															this.gameSettings.thirdPersonView = !this.gameSettings.thirdPersonView;
														}else {
															ThirdPersonTweaksHack.instance.thirdPersonEnabled = !ThirdPersonTweaksHack.instance.thirdPersonEnabled;
															if(!ThirdPersonTweaksHack.instance.thirdPersonEnabled && this.gameSettings.thirdPersonView) {
																this.gameSettings.thirdPersonView = !this.gameSettings.thirdPersonView;
															}
														}
													}else {
        ...
        if(CombatLogHack.shouldQuit) {
			...
			if(CombatLogHack.instance.mode.currentMode.equalsIgnoreCase("Health")) CombatLogHack.instance.status = false;
		}
